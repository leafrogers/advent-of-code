...
import run from './05.mjs';
import examples from './05.examples.mjs';
import inputs from './05.inputs.mjs';
...
describe('Day 5', () => {
	...
	* aren't sure which seat is yours, and all of the flight attendants are busy with the flood of
	...
	*     keeping rows 0 through 63.  B means to take the upper half, keeping rows 32 through 63.  F
	*     means to take the lower half, keeping rows 32 through 47.  B means to take the upper half,
	*     keeping rows 40 through 47.  B keeps rows 44 through 47.  F keeps rows 44 through 45.  The
	*     final F keeps the lower of the two, row 44.
	*
	...
	*     half, keeping columns 4 through 7.  L means to take the lower half, keeping columns 4
	*     through 5.  The final R keeps the upper of the two, column 5.
	*
	...
	*/
	describe('Part 1', () => {
		...
		it('matches the now known real answer', () => {
			...
			expect(highestSeatId).to.equal(926);
		});
	});
	...
	describe('Part 2', () => {
		it('matches the now known real answer', () => {
			...
			seatIds.forEach((seatId, index) => {
				if (index > 0 && (seatId - 1 !== seatIds[index - 1])) {
					mySeatId = seatId - 1;
				}
			});

			expect(mySeatId).to.equal(657);
		});
	});
});
