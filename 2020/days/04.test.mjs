...
import run from './04.mjs';
import examples from './04.examples.mjs';
import inputs from './04.inputs.mjs';
...
describe('Day 4', () => {
	...
	*
	*     byr (Birth Year) iyr (Issue Year) eyr (Expiration Year) hgt (Height) hcl (Hair Color) ecl
	*     (Eye Color) pid (Passport ID) cid (Country ID)
	*
	* Passport data is validated in batch files (your puzzle input). Each passport is represented as
	* a sequence of key:value pairs separated by spaces or newlines. Passports are separated by
	* blank lines.
	*
	...
	*/
	describe('Part 1', () => {
		it('matches the known answer in the example', () => {
			const passportStatuses = run({
				inputs: examples,
				useValidationRules: false
			});

			expect(passportStatuses).to.deep.equal([
				true,
				false,
				true,
				false
		...
		it('matches the now known real answer', () => {
			...
			expect(passportStatuses.filter(Boolean).length).to.equal(242);
		});
	});
	...
	*
	*     byr (Birth Year) - four digits; at least 1920 and at most 2002.  iyr (Issue Year) - four
	*     digits; at least 2010 and at most 2020.  eyr (Expiration Year) - four digits; at least
	*     2020 and at most 2030.  hgt (Height) - a number followed by either cm or in: If cm, the
	*     number must be at least 150 and at most 193.  If in, the number must be at least 59 and at
	*     most 76.  hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f.  ecl (Eye
	*     Color) - exactly one of: amb blu brn gry grn hzl oth.  pid (Passport ID) - a nine-digit
	*     number, including leading zeroes.  cid (Country ID) - ignored, missing or not.
	*
	...
	*/
	describe('Part 2', () => {
		it('matches the known answer in the example of invalid passports', () => {
			const answer = run({
...
pid:3556412378 byr:2007`
			...
			expect(answer).to.deep.equal([false, false, false, false]);
		});

		it('matches the known answer in the example of valid passports', () => {
			const answer = run({
				inputs: `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719`
			...
			expect(answer).to.deep.equal([true, true, true, true]);
		});

		it('matches the now known real answer', () => {
			const validPassports = run({
				inputs
			}).filter(Boolean);

			expect(validPassports.length).to.equal(186);
		});
	});
});
