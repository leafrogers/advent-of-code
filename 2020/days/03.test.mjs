...
import run from './03.mjs';
import examples from './03.examples.mjs';
import inputs from './03.inputs.mjs';
...
describe('Day 3', () => {
	...
	*/
	describe('Part 1', () => {
		it('matches the known answer in the example', () => {
			const answer = run({
				inputs: examples,
				moveRightEachTimeBy: 3,
				moveDownEachTimeBy: 1
			});
	...
	*
	*     Right 1, down 1.  Right 3, down 1. (This is the slope you already checked.) Right 5, down
	*     1.  Right 7, down 1.  Right 1, down 2.
	*
	...
	*/
	describe('Part 2', () => {
		it('matches the known answer in the example', () => {
			const treeCountsForSlopes = [];

			treeCountsForSlopes.push(run({
				inputs: examples,
				moveRightEachTimeBy: 1,
				moveDownEachTimeBy: 1
			}));
			...
			treeCountsForSlopes.push(run({
				inputs: examples,
				moveRightEachTimeBy: 5,
				moveDownEachTimeBy: 1
			}));

			treeCountsForSlopes.push(run({
				inputs: examples,
				moveRightEachTimeBy: 7,
				moveDownEachTimeBy: 1
			}));
			...
			treeCountsForSlopes.push(run({
				inputs: examples,
				moveRightEachTimeBy: 1,
				moveDownEachTimeBy: 2
			}));

			const treeCountsProduct = treeCountsForSlopes.reduce(
				(previous, current) => previous * current
			);

			expect(treeCountsProduct).to.equal(336);
		});
		...
		it('matches the now known real answer', () => {
			...
			expect(treeCountsProduct).to.equal(5872458240);
		});
	});
});
