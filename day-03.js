(() => {
	...
	const markSpot = ({
		...
		if (spot.marker === lineMapOpposites.get(direction) || spot.marker === CHAR_JUNCTION) {
			if (mostRecentWireTraversal && mostRecentWireTraversal.wireIndex !== wireIndex) {
				spot.marker = CHAR_OVERLAP;
			}
			// Otherwise keep the marker the same
		} else if (spot.marker === lineMap.get(direction)) {
			// Keep the marker the same
		} else if (isChangingDirectionNext) {
			spot.marker = CHAR_JUNCTION;
		} else if (spot.marker === CHAR_SPACE) {
	...
	const operateOnCode = ({
		...
		const distanceToTravelInThisDirection = Number(code.slice(1));
		const incrementDirection = operationMap.get(direction);

		// console.log('operating on code:', code, direction, num);

		for (
			let distanceSoFarInThisDirection = 0;
			distanceSoFarInThisDirection < distanceToTravelInThisDirection;
			distanceSoFarInThisDirection += 1
		) {
			incrementDirection();
			markSpot({
				direction,
				isChangingDirectionNext: (distanceSoFarInThisDirection === distanceToTravelInThisDirection - 1) && !isLastCode,
				wireDistanceTravelled: wireDistanceTravelled + distanceSoFarInThisDirection,
				wireIndex
			});
		}
	};

	const operateOnCodeArray = (codeArray, wireIndex) => {
		console.log(`Operating on code array of ${codeArray.length} codes, wire ${wireIndex}…`);
		...
		for (
			let codeIndex = 0, l = codeArray.length, wireDistanceTravelled = 1;
			codeIndex < l;
			codeIndex += 1, wireDistanceTravelled += Number(codeArray[codeIndex ? codeIndex - 1 : 0].slice(1))
		) {
			operateOnCode({
				code: codeArray[codeIndex],
				isLastCode: codeIndex === l - 1,
				wireDistanceTravelled,
				wireIndex
			});
		}
	...
	const getCrossings = () => {
		...
		for (
			...
			for (
				...
				if (spot.marker === CHAR_OVERLAP) {
					const xDelta = Math.abs(columnIndex - startX);
					const yDelta = Math.abs(rowIndex - startY);
					const stepsToOverlap = spot.history[0].wireDistanceTravelled + spot.history[1].wireDistanceTravelled;
